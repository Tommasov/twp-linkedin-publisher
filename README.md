...
- **Image Gallery Selection**: Pick which images to publish by ticking checkboxes. Detected images include the featured image, media uploaded to the article, and images referenced in the content — including **WPBakery** media grids/galleries (`vc_media_grid`, `vc_gallery`, `vc_single_image`) and `<img>` tags. You can also add **any image from the Media Library** (even one not shown in the article, uploaded just for LinkedIn) via the "Add image from library" button. Selected images are published as a multi-image LinkedIn post (max 9). If none are selected, the featured image is used as before.
...
4. (Optional) Tick the **images** you want to include from the article gallery (max 9), or click **Add image from library** to add an image that isn't in the article. Leave everything unchecked to use the featured image.
